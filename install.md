# profetional_ml_sklearn guide installation
...
activate profetional_ml_sklearn
```
...
To move beyond notebook prototyping, all reusable code should go into the `profetional_ml_sklearn/` folder package. To use that package inside your project, install the project's module in editable mode, so you can edit files in the `profetional_ml_sklearn` folder and use the modules inside your notebooks :
...
```python
from profetional_ml_sklearn.utils.paths import data_dir
data_dir()
```
